...
# bpy (Blender as a Python module) has no installable wheel for this Python/platform.
# It was only used by convert_obj_to_glb(), which is reimplemented below with trimesh.
# Guard the import so the module still loads; the legacy Blender helpers below remain
# available if bpy is ever present but are not used by the inference pipeline.
try:
    import bpy
except Exception:
    bpy = None
...
def convert_obj_to_glb(
    ...
    """Convert a textured OBJ (with PBR map sidecars written by save_obj_mesh) to a
    glTF-binary (.glb) with a PBR metallic-roughness material, using trimesh + PIL.

    This is a drop-in replacement for the original Blender-based implementation, which
    required the `bpy` package (unavailable as a wheel for this Python/platform). The
    sidecar maps emitted next to the OBJ are:
        <name>.jpg            -> baseColorTexture (albedo)
        <name>_metallic.jpg   -> grayscale metallic  (-> glTF MR texture, Blue channel)
        <name>_roughness.jpg  -> grayscale roughness (-> glTF MR texture, Green channel)
    ...
    try:
        ...
        # Load geometry + UVs without reprocessing (preserve vertex/UV correspondence).
        loaded = trimesh.load(obj_path, process=False, file_type="obj")
        if isinstance(loaded, trimesh.Scene):
            loaded = trimesh.util.concatenate(tuple(loaded.geometry.values()))
        uv = getattr(getattr(loaded, "visual", None), "uv", None)

        def _load(suffix, mode=None):
        ...
        albedo = _load("")
        metallic = _load("_metallic", "L")
        roughness = _load("_roughness", "L")
        normal = _load("_normal", "RGB")

        # glTF packs both factors into one texture: R unused, G=roughness, B=metallic.
        mr_texture = None
        if metallic is not None or roughness is not None:
            size = (metallic or roughness).size
            r_ch = Image.new("L", size, 255)
            g_ch = (roughness.resize(size) if roughness is not None else Image.new("L", size, 255))
            b_ch = (metallic.resize(size) if metallic is not None else Image.new("L", size, 0))
        ...
        if uv is not None:
            loaded.visual = TextureVisuals(uv=uv, material=material)

        loaded.export(glb_path)
        return True
    except Exception as exc:
        print(f"convert_obj_to_glb (trimesh) failed: {exc}")
        return False
